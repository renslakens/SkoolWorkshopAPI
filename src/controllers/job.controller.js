...
let controller = {
    ...
    deleteJob: (req, res) => {
        ...
        pool.query('DELETE FROM Opdracht WHERE jobID = ?;', [jobID], function(
            error,
            result,
        ) {
            if (error) {
    ...
    updateJob: (req, res) => {
        ...
        pool.query(
            'UPDATE Opdracht SET aantalDocenten=?, startTijd=?, eindTijd=? WHERE jobID =?;', [job.aantalDocenten, startTijd, eindTijd, jobID],
            function(error, result) {
                if (error) {
                    res.status(400).json({
                        status: 400,
    ...
    getWorkshops: (req, res, next) => {
        ...
        let queryString = 'SELECT O.opdrachtID, W.workshopnaam, W.beschrijving, O.aantalDocenten, O.startTijd, O.eindTijd, L.naam, L.land, L.postcode, L.straat, L.huisnummer, L.plaats, O.locatieID, O.workshopID, O.klantID, O.doelgroepID, DIO.loginEmail, DIO.isBevestigd FROM workshop W, Opdracht O, locatie L, DocentInOpdracht DIO'
    ...
    deleteTeacherFromJob: (req, res) => {
        ...
        pool.query(
            'DELETE FROM DocentInOpdracht WHERE docentID = ?', [docentID],
            function(error, result) {
                if (error) throw error
